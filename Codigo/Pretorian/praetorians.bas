...
errorh:
    Call LogError("Error en NPCAI.EsPretoriano? " & Npclist(NpcIndex).Name)
    'do nothing
...
On Error GoTo errorh
    ...
    Call SpawnNpc(PRKING_NPC, wp, True, True)
    'Call CrearNPC(PRKING_NPC, MAPA_PRETORIANO, wp)
    wp.X = wp.X + 3
    Call SpawnNpc(PRCLER_NPC, wp, True, True)
    'Call CrearNPC(PRCLER_NPC, MAPA_PRETORIANO, wp)
    wp.X = wp.X - 6
    Call SpawnNpc(PRCLER_NPC, wp, True, True)
    'Call CrearNPC(PRCLER_NPC, MAPA_PRETORIANO, wp)
    wp.Y = wp.Y + 3
    Call SpawnNpc(PRGUER_NPC, wp, True, True)
    ...
    wp.Y = wp.Y - 6
    wp.X = wp.X - 1
    Call SpawnNpc(PRCAZA_NPC, wp, True, True)
    'Call CrearNPC(PRCAZA_NPC, MAPA_PRETORIANO, wp)
    wp.X = wp.X - 4
    Call SpawnNpc(PRMAGO_NPC, wp, True, True)
    'Call CrearNPC(PRMAGO_NPC, MAPA_PRETORIANO, wp)
Exit Sub

errorh:
    Call LogError("Error en NPCAI.CrearClanPretoriano ")
    'do nothing
    ...
    For X = NPCPosX + (azar * 8) To NPCPosX + (azar * -8) Step -azar
        For Y = NPCPosY + (azar2 * 7) To NPCPosY + (azar2 * -7) Step -azar2
            ...
            If (PJEnInd > 0) And (Npclist(npcind).CanAttack = 1) Then
                If (UserList(PJEnInd).flags.Invisible = 0) And Not (UserList(PJEnInd).flags.Muerto = 1) And (UserList(PJEnInd).flags.Privilegios < 1 And EsAdmin(PJEnInd) = False) Then
                'ToDo: Borrar los GMs
...
Exit Sub
errorh:
    Call LogError("Error en NPCAI.PRCAZA_AI ")
    'do nothing
...
'        azar = 1
        ...
        For X = NPCPosX + (azar * 8) To NPCPosX + (azar * -8) Step -azar
            For Y = NPCPosY + (azar2 * 7) To NPCPosY + (azar2 * -7) Step -azar2
                Debuger = 7
                ' 0.12b2 fix
                ' Fix error del Mago choto
                If X < 1 Or X > 90 Then X = 1
                If Y < 1 Or Y > 90 Then Y = 1
                Debuger = 8
                NPCAlInd = MapData(NPCPosM, X, Y).NpcIndex  ''por si implementamos algo contra NPCs
                PJEnInd = MapData(NPCPosM, X, Y).UserIndex
                Debuger = 9
                If (PJEnInd > 0) And (Npclist(npcind).CanAttack = 1) And (UserList(PJEnInd).flags.Privilegios < 1 And EsAdmin(PJEnInd) = False) Then
...
errorh:
    Call LogError("Error en NPCAI.PRMAGO_AI? " & Err.Number & " - Debug:" & Debuger)
     'Error en NPCAI.PRMAGO_AI? 9 - Debug:2
...
errorh:
    Call LogError("Error en NPCAI.PRREY_AI? ")
...
On Error GoTo errorh
    ...
    For X = NPCPosX - 8 To NPCPosX + 8
        For Y = NPCPosY - 7 To NPCPosY + 7
            PJEnInd = MapData(NPCPosM, X, Y).UserIndex
            If (PJEnInd > 0) Then
                If (Not (UserList(PJEnInd).flags.Invisible = 1 Or UserList(PJEnInd).flags.Muerto = 1)) And EsAlcanzable(npcind, PJEnInd) And (UserList(PJEnInd).flags.Privilegios < 1 And EsAdmin(PJEnInd) = False) Then
...
errorh:
    Call LogError("Error en NPCAI.PRGUER_AI? ")
            ...
            If (Npclist(npcind).CanAttack = 1) Then   ''saltea el analisis si no puede atacar para evitar cuentas
                If (NPCAlInd > 0) Then  ''allie?
                    If (esPretoriano(NPCAlInd) = 0) Then
                        If Not (PJBestTarget) And (Npclist(NPCAlInd).MaestroUser > 0) And (Not (Npclist(NPCAlInd).flags.Paralizado > 0)) Then
                            BestTarget = NPCAlInd   ''lo mejor es una mascota
                            PJBestTarget = False
                            quehacer = 3
                        End If
                ...
                ElseIf (PJEnInd > 0) Then ''aggressor
                    If Not (UserList(PJEnInd).flags.Muerto = 1) And (UserList(PJEnInd).flags.Privilegios < 1 And EsAdmin(PJEnInd) = False) Then
...
errorh:
    Call LogError("Error en NPCAI.PRCLER_AI? ")
...
On Error GoTo errorh
    ...
    EsMagoOClerigo = UserList(PJEnInd).clase = CLASS_MAGO Or _
                        UserList(PJEnInd).clase = CLASS_CLERIGO Or _
                        UserList(PJEnInd).clase = CLASS_DRUIDA Or _
                        UserList(PJEnInd).clase = CLASS_BARDO
Exit Function

errorh:
    Call LogError("Error en NPCAI.EsMagoOClerigo? ")
End Function
...
errorh:
    Call LogError("Error en NPCAI.NPCRemueveVenenoNPC? ")
...
errorh:
    Call LogError("Error en NPCAI.NPCCuraLevesNPC? ")
...
errorh:
    Call LogError("Error en NPCAI.NPCRemueveParalisisNPC? ")
...
errorh:
    Call LogError("Error en NPCAI.NPCParalizaNPC? ")
...
errorh:
    Call LogError("Error en NPCAI.NPCcuraNPC? ")
...
errorh:
    Call LogError("Error en NPCAI.NPCLanzaCegueraPJ? ")
End Sub
...
errorh:
    Call LogError("Error en NPCAI.NPCLanzaEstupidezPJ? ")
End Sub
...
errorh:
    Call LogError("Error en NPCAI.NPCRemueveInvisibilidad ")
...
errorh:
    Call LogError("Error en NPCAI.NPCLanzaSpellSobreUser2 ")
            ...
            For X = 8 To 95
                For Y = 8 To 95
                    ...
                    If PJInd > 0 Then
                        ...
                        ''efectiviza el danio
                        If UserList(PJInd).flags.Privilegios = 0 Then
...
errorh:
    Call LogError("Error en NPCAI.MagoDestruyeWand ")
...
errorh:
    Call LogError("Error en NPCAI.GreedyWalkTo")
...
errorh:
    Call LogError("Error en NPCAI.MoverAba ")
...
errorh:
    Call LogError("Error en NPCAI.MoverArr")
End Sub
...
errorh:
    Call LogError("Error en NPCAI.MoverIzq")
...
errorh:
    Call LogError("Error en NPCAI.MoverDer")
...
errorh:
    Call LogError("Error en NPCAI.VolverAlCentro")
...
errorh:
    Call LogError("Error en NPCAI.EstoyLejos")
...
errorh:
    Call LogError("Error en NPCAI.EsAlcanzable")
...
'    MsgBox ("ERROR!!")
    CasperBlock = False
    Call LogError("Error en NPCAI.CasperBlock")
...
errorh:
    Call LogError("Error en NPCAI.LiberarCasperBlock")
