            ...
            If UI > 0 Then
                  If UserList(UI).flags.Muerto = 0 Then
                        ...
                        ' [GS] No serguir o mirar a a GM's
                         If (UserList(UI).flags.Privilegios > 1 Or EsAdmin(UI)) Or UserList(UI).flags.PocionRepelente = True Then GoTo Sig
                         ' [/GS]
                         If Not UserList(UI).Faccion.FuerzasCaos Then
                            If Criminal(UI) Then
...
Private Sub GuardiasDarkAI(ByVal NpcIndex As Integer)
Dim nPos As WorldPos
Dim headingloop As Byte
Dim tHeading As Byte
Dim Y As Integer
...
For headingloop = NORTH To WEST
    ...
    If Npclist(NpcIndex).flags.Inmovilizado = 0 Or Npclist(NpcIndex).Char.Heading = headingloop Then
        ...
        If InMapBounds(nPos.Map, nPos.X, nPos.Y) Then
            ...
            If UI > 0 Then
                ' [GS] No serguir o mirar a a GM's
                If (UserList(UI).flags.Privilegios > 1 Or EsAdmin(UI)) Or UserList(UI).flags.PocionRepelente = True Then GoTo Siguiente:
                ' [/GS]
                If UserList(UI).flags.Muerto = 0 Then
    ...
    For Y = Npclist(NpcIndex).Pos.Y To Npclist(NpcIndex).Pos.Y + SignoNS * 10 Step IIf(SignoNS = 0, 1, SignoNS)
        For X = Npclist(NpcIndex).Pos.X To Npclist(NpcIndex).Pos.X + SignoEO * 10 Step IIf(SignoEO = 0, 1, SignoEO)
            ...
            If X >= MinXBorder And X <= MaxXBorder And Y >= MinYBorder And Y <= MaxYBorder Then
                   ...
                   If UI > 0 Then
                        ' [GS] No serguir o mirar a a GM's
                        If (UserList(UI).flags.Privilegios > 1 Or EsAdmin(UI)) Or UserList(UI).flags.PocionRepelente = True Then GoTo Siguiente2:
                        ' [/GS]
        ...
        For X = Npclist(NpcIndex).Pos.X - 10 To Npclist(NpcIndex).Pos.X + 10
            If X >= MinXBorder And X <= MaxXBorder And Y >= MinYBorder And Y <= MaxYBorder Then
                   ...
                   If UI > 0 Then
                        ...
                        ' [/GS]
                      If UserList(UI).flags.Muerto = 0 Then
                           If UserList(UI).flags.Invisible = 0 Or (UserList(UI).flags.Invisible = 1 And Npclist(NpcIndex).flags.AtacaInvis = True) Then
        ...
        For X = Npclist(NpcIndex).Pos.X - 10 To Npclist(NpcIndex).Pos.X + 10
            If X >= MinXBorder And X <= MaxXBorder And Y >= MinYBorder And Y <= MaxYBorder Then
                ...
                If UI > 0 Then
                    ...
                    ' [/GS]
                    If UserList(UI).Name = Npclist(NpcIndex).flags.AttackedBy Then
                        If UserList(UI).flags.Muerto = 0 Then ' No ataca muertos viteh
                                If UserList(UI).flags.Invisible = 0 Or (UserList(UI).flags.Invisible = 1 And Npclist(NpcIndex).flags.AtacaInvis = True) Then
                                    If Npclist(NpcIndex).flags.LanzaSpells > 0 Then
                                         Dim k As Integer
                                         k = RandomNumber(1, Npclist(NpcIndex).flags.LanzaSpells)
            ...
            If X >= MinXBorder And X <= MaxXBorder And Y >= MinYBorder And Y <= MaxYBorder Then
               ...
               If NI > 0 Then
                    If Npclist(NpcIndex).TargetNPC = NI Then
                         ...
                         ' mirar al npc (v0.12b1)
                         tHeading = FindDirection(Npclist(NpcIndex).Pos, Npclist(NI).Pos)
                         Call HeadtoPos(tHeading, Npclist(NpcIndex).Pos)
...
On Error GoTo ErrorHandler
        If Npclist(NpcIndex).Meditando = True Then Exit Function
        '<<<<<<<<<<< Ataques >>>>>>>>>>>>>>>>
        If Npclist(NpcIndex).MaestroUser = 0 Then
        ...
        Select Case Npclist(NpcIndex).Movement
            Case MUEVE_AL_AZAR
                ...
                ElseIf Npclist(NpcIndex).NPCtype = NPCTYPE_GUARDIASKAOS Then ' [NEW]
                    If Int(RandomNumber(1, 12)) = 3 Then
                        Call MoveNPCChar(NpcIndex, CByte(RandomNumber(1, 4)))
                    End If
            ...
            Case GUARDIAS_ATACAN_CRIMINALES
                Call PersigueCriminal(NpcIndex)
            Case GUARDIAS_ATACAN_CIUDADANOS     ' [NEW]
                Call PersigueCiudadano(NpcIndex)    ' [NEW]
            Case SIGUE_AMO
                If Npclist(NpcIndex).flags.Inmovilizado = 1 Then Exit Function
...
ErrorHandler:
    If FileExist(App.Path & "\Logs\NPCAI.log", vbArchive) Then
        If FileSize(App.Path & "\Logs\NPCAI.log") > 29999 Then ' Mayor a 30k
            Call BorrarArchivo(App.Path & "\Logs\NPCAI.log")
        End If
    End If
    Call LogGM("NPCAI", "NPCAI " & Npclist(NpcIndex).Name & " " & Npclist(NpcIndex).MaestroUser & " " & Npclist(NpcIndex).MaestroNpc & " mapa:" & Npclist(NpcIndex).Pos.Map & " x:" & Npclist(NpcIndex).Pos.X & " y:" & Npclist(NpcIndex).Pos.Y & " Mov:" & Npclist(NpcIndex).Movement & " TargU:" & Npclist(NpcIndex).Target & " TargN:" & Npclist(NpcIndex).TargetNPC, False)
    Call ResetNPC(NpcIndex)
End Function
         ...
         If X > MinXBorder And X < MaxXBorder And Y > MinYBorder And Y < MaxYBorder Then
             ...
             If MapData(Npclist(NpcIndex).Pos.Map, X, Y).UserIndex > 0 Then
                 'Move towards user
                  ...
                  'routines.
                ' [GS] No serguir o mirar a a GM's
                If (UserList(tmpUserIndex).flags.Privilegios > 1 Or EsAdmin(tmpUserIndex)) Or UserList(tmpUserIndex).flags.PocionRepelente = True Then GoTo siguienteNW
                ' [/GS]
                ...
                If UserList(tmpUserIndex).flags.Muerto = 0 Then
                    If Npclist(NpcIndex).flags.LanzaSpells <> 0 Then
                        If UserList(tmpUserIndex).Stats.UserAtributos(Suerte) = 0 Or RandomNumber(1, UserList(tmpUserIndex).Stats.UserAtributos(Suerte) + 10) > UserList(tmpUserIndex).Stats.UserAtributos(Suerte) Then
                            Call NpcLanzaUnSpell(NpcIndex, tmpUserIndex)
                        End If
                    End If
                End If
                ' [/GS]
...
' 0.12b4
If Distancia(Npclist(NpcIndex).Pos, UserList(UserIndex).Pos) > 17 Then Exit Sub
